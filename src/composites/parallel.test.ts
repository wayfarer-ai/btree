...
class DelayedAction extends ActionNode {
  ...
  executeTick(_context: EffectTickContext) {
    ...
    return Effect.gen(function* (_) {
      self.log(
        `Tick ${self.totalTicks - self.ticksRemaining + 1}/${self.totalTicks}`,
      );
      ...
      if (self.ticksRemaining > 0) {
        ...
        if (self.ticksRemaining === 0) {
          self.log(`Completing with ${self.finalStatus}`);
...
class TimestampedAction extends ActionNode {
  ...
  executeTick(_context: EffectTickContext) {
    ...
    return Effect.gen(function* (_) {
      ...
      // Simulate async work
      yield* _(Effect.sleep(`${self.delay} millis`));

      self.completionTime = Date.now();
      return self.finalStatus;
    });
  }
...
describe("Parallel - Concurrent Execution", () => {
  ...
  describe("Basic Behavior", () => {
    it.live("should return SUCCESS when empty", () =>
      Effect.gen(function* (_) {
        const parallel = new Parallel({ id: "test-parallel" });
        const status = yield* _(parallel.tick(context));
        expect(status).toBe(NodeStatus.SUCCESS);
      }),
    );

    it.live("should execute all children concurrently (not sequentially)", () =>
      Effect.gen(function* (_) {
        const parallel = new Parallel({ id: "test-parallel" });
    ...
    it.live(
      ...
      () =>
        Effect.gen(function* (_) {
          ...
          const status1 = yield* _(parallel.tick(context));
          expect(status1).toBe(NodeStatus.RUNNING);
        }),
    );

    it.live("should continue tracking state across multiple ticks", () =>
      Effect.gen(function* (_) {
        const parallel = new Parallel({ id: "test-parallel" });
        ...
        const status2 = yield* _(parallel.tick(context));
        expect(status2).toBe(NodeStatus.RUNNING);

        // Tick 3: Both done
        const status3 = yield* _(parallel.tick(context));
        expect(status3).toBe(NodeStatus.SUCCESS);
      }),
    );

    it.live("should retick children that return RUNNING", () =>
      Effect.gen(function* (_) {
        const parallel = new Parallel({ id: "test-parallel" });
        ...
        yield* _(parallel.tick(context)); // Tick 1: Launch child (returns RUNNING)
        expect(child1ExecutionCount).toBe(1);

        yield* _(parallel.tick(context)); // Tick 2: Retick child (completes)
  ...
  describe("Strategy: strict (all must succeed)", () => {
    it.live("should succeed when all children succeed", () =>
      Effect.gen(function* (_) {
    ...
    it.live("should fail if any child fails", () =>
      Effect.gen(function* (_) {
        const parallel = new Parallel({
          id: "test-parallel",
          strategy: "strict",
    ...
    it.live("should use strict strategy by default", () =>
      Effect.gen(function* (_) {
    ...
    it.live("should wait for all children to complete before returning", () =>
      Effect.gen(function* (_) {
        const parallel = new Parallel({
          id: "test-parallel",
          strategy: "strict",
  ...
  describe("Strategy: unknown (at least one must succeed)", () => {
    it.live("should succeed if at least one child succeeds", () =>
      Effect.gen(function* (_) {
        const parallel = new Parallel({
          id: "test-parallel",
          strategy: "any",
    ...
    it.live("should fail if all children fail", () =>
      Effect.gen(function* (_) {
        const parallel = new Parallel({
          id: "test-parallel",
          strategy: "any",
    ...
    it.live("should still wait for all children to complete", () =>
      Effect.gen(function* (_) {
        const parallel = new Parallel({
          id: "test-parallel",
          strategy: "any",
  ...
  describe("Error Handling", () => {
    it.live("should treat thrown errors as FAILURE", () =>
      Effect.gen(function* (_) {
        const parallel = new Parallel({
          id: "test-parallel",
          strategy: "strict",
        ...
        // Effect.fail errors propagate - wrap in Effect.exit to catch
    ...
    it.live("should continue executing other children when one throws", () =>
      Effect.gen(function* (_) {
        const parallel = new Parallel({
          id: "test-parallel",
          strategy: "any",
        ...
        // So the parallel node will complete with a status based on child statuses
        const status = yield* _(parallel.tick(context));
  ...
  describe("State Management", () => {
    it.live("should reset state on halt", () =>
      Effect.gen(function* (_) {
        const parallel = new Parallel({ id: "test-parallel" });
    ...
    it.live("should reset state on reset", () =>
      Effect.gen(function* (_) {
        const parallel = new Parallel({ id: "test-parallel" });
  ...
  describe("Real-World Scenario: WaitForNewPage + Click", () => {
    it.live("should handle Playwright pattern correctly", () =>
      Effect.gen(function* (_) {
        const parallel = new Parallel({
          id: "wait-and-click",
          strategy: "strict",
